...
- Развивать **единый shared‑слой UI/компонентов** (`packages/ui` → `@synestra/ui/*`) и давать сайтам **безопасный механизм точечных отличий** через app‑facade `@/ui/*` (file‑overrides без форков).
...
- Окружения проектируем как `dev → stage → prod`, но на старте используем **`dev` + `prod`** (без `stage`): dev для hot‑разработки, prod для стабильного baseline и домена.
- Hot‑разработка в Kubernetes: выбран подход через **Okteto** (Self‑Hosted уже развернут в `synestra-platform`; канон использования фиксируем в runbook’ах).
