...
import type { PluginInstructionsData, PluginInstructionsDoc } from './plugin-ai-types.js'
...
export const init = async (
  ...
  const { docs: allInstructions } = (await payload.find({
    ...
    pagination: false,
    select,
  })) as { docs: PluginInstructionsDoc[] }

  const fieldInstructionsMap: Record<string, { fieldType: string; id: string | number }> = {}
  ...
  for (let i = 0; i < paths.length; i++) {
    ...
    if (!instructions) {
      ...
      const data: PluginInstructionsData = {
      ...
      instructions = (await payload
        ...
        })) as PluginInstructionsDoc | undefined
