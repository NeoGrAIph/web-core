import type { CollectionConfig, CollectionSlug, Config, GlobalConfig } from 'payload'
...
const payloadAiPlugin =
  ...
  (incomingConfig: Config): Config => {
    ...
    if (isActivated) {
      ...
      const collectionSlugs = pluginConfig.collections ?? {}
      const globalsSlugs = pluginConfig.globals ?? {}

      const { components: { providers = [] } = {} } = incomingConfig.admin || {}
      const updatedProviders = [
        ...(providers ?? []),
        {
          path: '@/providers/InstructionsProvider/InstructionsProvider#InstructionsProvider',
        },
      ]
      ...
      updatedConfig = {
        ...
        collections: collections.map((collection) => {
          ...
          if (collectionSlugs[collectionSlug]) {
        ...
        globals: globals.map((global) => {
          if (globalsSlugs && globalsSlugs[global.slug as keyof typeof globalsSlugs]) {
