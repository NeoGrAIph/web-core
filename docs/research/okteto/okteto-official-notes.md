...
1) Локация и формат okteto‑манифеста:
   ...
   - у нас в runbook описана идея `apps/<app>/okteto.yml` — стоит решить, какой стандарт мы фиксируем для монорепы.

2) File sync:
   - добавить в runbook практики `.stignore` и `okteto status --info` как обязательную диагностику.
...
1) На основании этого конспекта — обновить/уточнить `docs/runbooks/runbook-okteto-dev.md`:
   - добавить официальные команды/паттерны диагностики (Syncthing/.stignore),
   - зафиксировать стандарт расположения `okteto.yml` в монорепе,
   - добавить минимальный “эталон” okteto‑manifest для `apps/synestra-io` (пока без внедрения в кластер).
2) Затем переходить к Argo CD докам уже “в контексте Okteto”: как правильно делать ignoreDifferences, drift, selfHeal и т.п.
