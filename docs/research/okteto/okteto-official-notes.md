...
Операционная находка (из практики, важно для runbook’ов):
- Dev namespaces, в которых мы хотим запускать `okteto up`, должны быть **созданы через Okteto** (`okteto namespace create ...`), иначе Okteto CLI может не видеть существующий Kubernetes namespace.
- Значение `cluster.endpoint` в values Okteto должно совпадать с SAN сертификата Kubernetes API server; иначе `okteto up` падает с `x509` hostname mismatch.
...
2) File sync:
   ...
   - по опыту добавили отдельные исключения для PVC mount (например media‑директории) и для `.pnpm-store`, иначе Syncthing может “ломаться”.
