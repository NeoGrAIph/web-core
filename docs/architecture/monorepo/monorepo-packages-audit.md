...
- `turbo/` как зона “инструментов” уже существует, генераторы подключены (см. `turbo/generators/config.ts`), но шаблоны могут требовать обновления по текущим контрактам.
