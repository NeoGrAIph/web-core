...
Связанные документы:
- `docs/architecture/monorepo/monorepo-packages-standards.md` (решения: source‑packages, ESM, таксономия, зависимости)
...
- `package.json`
  ...
  - `scripts`:
    - `build` = typecheck без emit (для source‑packages)
