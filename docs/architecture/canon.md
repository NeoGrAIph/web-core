...
- Структура репозитория (кратко): `README.md`
- Контракт env vars: `docs/runbooks/runbook-env-contract.md`
- Runbooks (операционные): `docs/runbooks/` (см. реестр в `AGENTS.md`)
- Официальные конспекты: `docs/research/**` и `synestra-platform/docs/wiki/okteto.md` (раздел 10)
...
### 2.5. Фасады и admin‑слой обязательны

- UI в app‑коде импортируется **только** через `@/ui/*` (фасад).
- Admin UI отделён: кастомные admin‑компоненты подключаем через `@/admin-ui/*` и import map Payload.
- Shared‑пакеты не импортируются напрямую из app‑кода.
...
Нормативное описание структуры и маппинга на deployments: `README.md`.
...
Контракт и правила валидации: `docs/runbooks/runbook-env-contract.md`.
...
Нормативный документ по миграциям (Postgres): `docs/runbooks/runbook-payload-migrations.md`.
...
- **prod эталон (цель)**: `apps/payload-core` → `web-payload-core` → `https://payload.services.synestra.tech`
...
4. Стандарты структуры/генераторы (Turborepo generators) для быстрого добавления сайтов и типовых “блоков‑конструктора”.
   Короткое решение: **генератор обязателен после стабилизации UI‑контрактов**.
