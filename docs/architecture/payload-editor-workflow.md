...
- Seed/defaultValue и изменения schema: `docs/runbooks/runbook-payload-seeding.md`
...
## 2) Preview (internal preview) — frontend‑роуты

### 2.1. Контракт URL

Канон `web-core` (Next.js App Router, frontend‑группа роутов):
- enable: `GET /next/preview?collection=<slug>&slug=<slug>&path=<relative>`
- disable: `GET /next/exit-preview`

Примечание:
- в официальном шаблоне `website` эти маршруты находятся в `src/app/(frontend)/next/*/route.ts`;
- это frontend‑роуты и не относятся к `src/app/(payload)/**`.
...
Важно:
- `CRON_SECRET` должен храниться в Kubernetes Secret и быть частью env‑контракта (см. `docs/runbooks/runbook-env-contract.md`), чтобы избежать дублирования.
