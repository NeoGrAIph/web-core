...
Опорные элементы (без привязки к путям):
- каталог schema (например `PAGE_LAYOUT_BLOCKS`);
- registry рендера (например `PAGE_BLOCK_COMPONENTS`);
- связка/валидация через общий renderer (например `@synestra/blocks-renderer`).
...
Опорные элементы (без привязки к путям):
- каталог schema для rich text editor (например `RICH_TEXT_BLOCKS`);
- converters для рендера rich text (например `RICH_TEXT_JSX_CONVERTERS`);
