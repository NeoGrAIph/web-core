...
- Seed/defaultValue + изменения schema: `docs/architecture/payload-seeding.md`
...
- app‑локальные блоки (пока нестабильны или завязаны на домен/коллекции): `apps/<app>/src/blocks/<BlockName>/config.ts`
- wrapper (по умолчанию): `apps/<app>/src/blocks/<BlockName>/config.ts` → `export { ... } from '@synestra/cms-blocks'`
...
- `templates/payload/website/src/blocks/pageBuilder.ts` — аналог для шаблона
...
### 2.5. Embedded blocks в rich text (Lexical `BlocksFeature`)
...
Канон `web-core`:
- embedded blocks имеют **отдельный каталог** (`apps/<app>/src/blocks/richTextCatalog.ts`) и **отдельные converters** для рендера (`apps/<app>/src/blocks/richTextRenderers.tsx`);
...
### 2.4. Overrides (точечные кастомизации)
