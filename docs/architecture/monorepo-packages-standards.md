...
Связанные документы:
- Аудит “как сейчас”: `docs/architecture/monorepo-packages-audit.md`
...
- `turbo run build` остаётся частью пайплайна, но **build‑артефакты пакета не являются runtime‑источником истины** (runtime идёт из `src/*`, собранного Next’ом).
