...
- `docs/architecture/ci-cd-contract.md` — self-host CI/CD контракт (границы web-core ↔ platform, immutable tags, ArgoCD rollout)
- `docs/architecture/database-cnpg.md` — канон по БД Postgres через CloudNativePG (CNPG) для web‑приложений
- `docs/architecture/payload-migrations.md` — канон миграций Payload CMS 3 для PostgreSQL (baseline миграция, где и как запускать в prod)
