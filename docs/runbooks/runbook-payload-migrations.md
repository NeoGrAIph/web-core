...
## 5.1) Bootstrap‑миграции и идемпотентность (важно для Argo CD hook)

В `web-core` миграции запускаются как **Argo CD Sync hook** (`deploy/charts/web-app/templates/migrations-job.yaml`).
Это означает, что при **каждом sync** Job запускается снова.

Для baseline/bootstrapping миграций (которые создают всю схему с нуля) это может привести к падениям,
если в БД уже существуют enum‑типы/таблицы/индексы. Чтобы sync был повторяемым:

- Для bootstrap‑миграций используйте идемпотентные конструкции:
  - `CREATE TYPE IF NOT EXISTS`
  - `CREATE TABLE IF NOT EXISTS`
  - `CREATE INDEX IF NOT EXISTS` / `CREATE UNIQUE INDEX IF NOT EXISTS`

Практическое правило:
- **Baseline/initial** миграции делаем идемпотентными (безопасный повторный запуск).
- **Обычные инкрементальные** миграции оставляем строгими, чтобы ошибки не скрывались.

Если вы не хотите повторный запуск миграций при каждом sync, можно временно выключать их через values
(`migrations.enabled: false`) и включать только при появлении новых миграций.
