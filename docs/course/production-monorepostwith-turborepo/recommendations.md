...
- `globalDependencies` задаёт файлы, которые инвалидируют кеш “везде” (пример: `.env.*local`).

**Отражено ли уже в `web-core`**
- Да: `turbo.json` уже содержит `dependsOn` и `outputs` для `build`, а также `globalDependencies` для `.env.*local`.
